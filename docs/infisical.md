...
| `NTFY_DOMAIN` | Bare public ntfy hostname, for example `push.example.com` |
...
| `N8N_API_KEY` | Optional instance-owner n8n API key used to provision the Error Workflow after initial owner setup |
| `NTFY_TOKEN` | Shared ntfy access token; generate with `ntfy token generate` |
| `NTFY_USER_PASSWORD_HASH` | bcrypt hash from `ntfy user hash` for the declarative ntfy user |
